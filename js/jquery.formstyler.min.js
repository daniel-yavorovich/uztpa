...
a.innerWidth()>B&&(B=a.innerWidth(),H=a.width());a.css({display:""})});p=f.clone().appendTo("body").width("auto");h=p.find("select").outerWidth();p.remove();h==f.width()&&v.width(H);B>f.width()&&k.width(B);v.is(".placeholder")&&v.width()>B&&v.width(v.width());""===g.first().text()&&""!==a.data("placeholder")&&m.first().hide();a.css({position:"absolute",left:0,top:0,width:"100%",height:"100%",opacity:0});var I=f.outerHeight()+19,C=z.outerHeight(),D=r.css("max-height"),p=m.filter(".selected");1>p.length&&
